...
import { editResa } from './planning/edit_resa';
...
async function retrieveDataAjax() {
    ...
    try {
        ...
        var object_max_date = StringDateToObject(dataWithoutParent[0].end_date_formated);

        for (var j = 1; j < dataWithoutParent.length; j++) {
            var object_date = StringDateToObject(dataWithoutParent[j].end_date_formated);
