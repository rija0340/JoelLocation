...
class ModernPlanning {
    ...
    createVehicleRow(vehicle) {
        ...
        row.className = 'vehicle-row';
        row.dataset.vehicleId = vehicle.id;
        row.style.position = 'relative'; // Enable absolute positioning for child elements
        row.style.minHeight = '40px'; // Ensure minimum height for reservations
    ...
    addReservationsWithOverlapHandling(row, reservations) {
        ...
        // Adjust row height based on number of lanes
        if (lanes.length > 1) {
            row.style.height = `${lanes.length * 40}px`;
        }
    }
    ...
    addReservationToRow(row, reservation, laneIndex = 0) {
        ...
        const laneHeight = 35; // Height of each lane in pixels
