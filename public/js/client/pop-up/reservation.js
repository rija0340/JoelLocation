...
function getElements() {
    ...
    btnValiderDevis = document.querySelectorAll('button[id="validerDevis"]');
...
function addEventListener() {
    ...
    //valider devis dans section reservation > devis
    for (let i = 0; i < btnValiderDevis.length; i++) {
        btnValiderDevis[i].addEventListener("mouseover", showPopUp, false);
        btnValiderDevis[i].addEventListener("mouseout", hidePopUp, false);
    }
...
function createSpan(element) {
    ...
    if (element.id == "validerDevis") {
        span.innerText = "Valider le devis";
    }
